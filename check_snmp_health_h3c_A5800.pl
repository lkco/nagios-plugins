...
my $o_entPhysicalName      = "1.3.6.1.2.1.47.1.1.1.1.7";
my $o_EntityExtCpuUsage    = "1.3.6.1.4.1.25506.2.6.1.1.1.1.6";
my $o_EntityExtMemUsage    = "1.3.6.1.4.1.25506.2.6.1.1.1.1.8";
my $o_EntityExtTemperature = "1.3.6.1.4.1.25506.2.6.1.1.1.1.12";
my $keyword_match          = "board";  # value in o_entPhysicalName to catch cpu index
...
sub help {
   print "\nMonitor SNMP CPU HP H3C A5800 for Nagios version ",$Version,"\n";
...
my $resultat = (Net::SNMP->VERSION < 4) ?
                  $session->get_table($o_entPhysicalName)
                : $session->get_table(Baseoid => $o_entPhysicalName);
...
if ($o_mode eq "cpu-usage") {
    ...
    foreach my $key ( sort keys %$resultat) {
       if ($$resultat{$key} =~ /$keyword_match/i ) { 
           ...
           $key =~ /\.(\d+)$/ ;
           verb("Index = $1");
           $h_cpuoid{"cpustack$i"}= $o_EntityExtCpuUsage.".$1";
           $i++;
       }
    }
    ...
    foreach my $key ( sort keys %$resultat) {
       if ($$resultat{$key} =~ /$keyword_match/i ) { 
           ...
           $key =~ /\.(\d+)$/ ;
           verb("Index = $1");
           $h_memoid{"memstack$i"}= $o_EntityExtMemUsage.".$1";
           $i++;
       }
    }
    ...
    while (my ($key,$value) = each %h_memoid) {
        $h_memvalue{$key} = $$resultat{$value};
    }
    
    #print Dumper \%h_memvalue;
    
    my $perfdata = "";
    my $exit_val = $ERRORS{"OK"};
    my $state = "OK";
    my $message = "";
    foreach my $key ( sort keys %h_memvalue) {
        if ($h_memvalue{$key} > $o_crit) {
            $exit_val = $ERRORS{"CRITICAL"};
            $state = "CRITICAL";
        } elsif ($h_memvalue{$key} > $o_warn) {
            $exit_val = $ERRORS{"WARNING"};
            $state = "WARNING";
        }
        $message .= $key."=".$h_memvalue{$key}."% ";
        $perfdata .= $key."=".$h_memvalue{$key}."%;$o_warn;$o_crit;0;100 ";
    ...
    foreach my $key ( sort keys %$resultat) {
       if ($$resultat{$key} =~ /$keyword_match/i ) { 
           ...
           $key =~ /\.(\d+)$/ ;
           verb("Index = $1");
           $h_temperatureoid{"temperaturestack$i"}= $o_EntityExtTemperature.".$1";
           $i++;
       }
    }
    #print Dumper \%h_temperatureoid;
    
    my @oidlists = values %h_temperatureoid;
    $resultat = (Net::SNMP->VERSION < 4) ?
              $session->get_request(@oidlists)
            : $session->get_request(-varbindlist => \@oidlists);
    ...
    my %h_temperaturevalue;
    while (my ($key,$value) = each %h_temperatureoid) {
        $h_temperaturevalue{$key} = $$resultat{$value};
    }
    
    #print Dumper \%h_temperaturevalue;
    
    my $perfdata = "";
    my $exit_val = $ERRORS{"OK"};
    my $state = "OK";
    my $message = "";
    foreach my $key ( sort keys %h_temperaturevalue) {
        if ($h_temperaturevalue{$key} > $o_crit) {
            $exit_val = $ERRORS{"CRITICAL"};
            $state = "CRITICAL";
        } elsif ($h_temperaturevalue{$key} > $o_warn) {
            $exit_val = $ERRORS{"WARNING"};
            $state = "WARNING";
        }
        $message .= $key."=".$h_temperaturevalue{$key}."% ";
        $perfdata .= $key."=".$h_temperaturevalue{$key}."%;$o_warn;$o_crit ";
