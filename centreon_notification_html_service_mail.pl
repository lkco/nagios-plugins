#!/usr/bin/perl -w 
#
# ########## centreon_notification_html_service_mail.pl ############### #
# Date    : Juil 4, 2017                                                #
...
# Help    : ./centreon_notification_html_service_mail.pl -h             #
#                                                                       #
...
# Version : 1.9 Add compatibility with Centreon 2.8                     #
#                                                                       #
...
my $mail_sender        = "Engine Monitoring <monitoring\@mydomain.com>";
# The Nagios CGI URL for integrated service links
my $centreon_url      = "http://10.93.10.245/centreon";
...
# image generation. Use autologin feature to get image
# if not used we can set $png_centreon_url = undef;
# ########################################################################
my $autologin_centreon = "admin"
my $token_autologin = "wHj2cJ7SGy"
my $png_centreon_url     = $centreon_url."/include/views/graphs/generateGraphs/generateImage.php?autologin=1&username=".$autologin_centreon."&token=".$token_autologin;
...
my $domain 	       = "\@domain.com"; # this is only for -g groups
my @listaddress        = ();
...
# ########################################################################
my $logofile = "/usr/lib/nagios/plugins/gfi/mylogo.jpg";
...
my %language = ('en' => { 'A' => 'Customer',
                          ...
                          'L' => 'Author',
                          'M' => 'Comment',
                          'N' => 'Monitoring System Notification',
                          'O' => 'Generated by Centreon Engine ('.$poller_name.')' },
                'fr' => { 'A' => 'Client',
                          ...
                          'M' => 'Commentaire',
                          'N' => 'Monitoring System Notification',
                          'O' => 'Généré par Centreon Engine ('.$poller_name.')' },
                'de' => { 'A' => 'Anwender',
                          'B' => 'Nachrichtentyp',
                          'C' => 'Dienstname',
                          'D' => 'Dienstzustand',
...
sub check_options {
  ...
  GetOptions(
      ...
      'l:s'   => \$o_language,        'language:s'        => \$o_language,
                                      'notificationtype:s' => \$o_notificationtype,
                                      'servicedesc:s'      => \$o_servicedesc,
                                      'hostname:s'        => \$o_hostname,
                                      'hostalias:s'        => \$o_hostalias,
                                      'hostaddress:s'      => \$o_hostaddress,
                                      'serviceoutput:s'    => \$o_serviceoutput,
                                      'servicestate:s'     => \$o_servicestate,
                                      'datetime:s'         => \$o_datetime
  ) or unknown_arg();
...
sub create_message_html {
  ...
  $html_msg .= "<th width=$header_size class=even>$language{$land}{'B'}:</th>\n"
            ...
            . "<tr><th class=odd>$language{$land}{'F'}:</th><td>$o_serviceoutput</td></tr>\n";
  ...
  $html_msg .= "<tr><th class=odd>$language{$land}{'D'}:</th><td bgcolor=$cellcolor>$o_servicestate</td></tr>\n"
            . "<tr><th class=even>$language{$land}{'C'}:</th><td class=even>\n";
  ...
  if (defined($o_addurl)) {
    $html_msg .= "<a href=\"$centreon_url/main.php?p=20201&o=svcd&host_name=" . urlencode($o_hostname) 
              . "&service_description=" . urlencode($o_servicedesc) . "\">$o_servicedesc</a>\n";
    ...
    if ($o_format eq "graph" && $graph_type ne "gif") {
      ...
      $html_msg .=  " <a href=\"".$centreon_url."/main.php?p=20201&o=svcd&host_name=" . urlencode($o_hostname)
                . "&service_description=" . urlencode($o_servicedesc) . "\">"
  ...
  if (defined($o_addurl)) {
    $html_msg .= "<a href=\"$centreon_url/main.php?p=20201&o=svc&host_search=" . urlencode($o_hostname) 
              . "\">$o_hostname</a>";
  }
  ...
  $html_msg .= "<tr><th class=odd>$language{$land}{'K'}:</th><td>$o_datetime</td></tr>\n";
...
sub import_pnp_graph { 
  ...
  $img_get = $png_centreon_url."&hostname=" . urlencode($o_hostname) . "&service=" 
           . urlencode($o_servicedesc) . "";

  my $res = $ua->get($img_get);
  if (($res->is_success) && length($res->content) > 20) {
    verb("import_pnp_graph: Length : ".length($res->content)."\n");
